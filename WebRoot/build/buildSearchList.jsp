...
<tbody>
<c:forEach var="item" items="${list.date}">
<tr onmousemove="this.className='lightup';" onmouseout="this.className='lightdown';"	>
<td><input name="checkone" type="checkbox" value="${item.id }" /></td>
<td>${item.buildName }</td>
<td>${item.buildStart }</td>
<td>${item.buildFinish }</td>
<td>${item.buildLeak }</td>
<td>${item.buildArea }</td>

<td><a href="../build!readyUpdate.do?id=${item.id }">edit</a></td>
...
<tr>
         ...
         <ul>                            <li><a href="../build!find.do?pageNum=1">首页</a></li>
                            <li><a href="../build!find.do?pageNum=${list.previous }">前页</a></li>
